...
class MedicGrenadeExplosionDamageArea : ExplosionDamageArea
{
	...
	private void Awake()
	{
		...
		}else
        {
			...
			//because when you set an EntityAlive's MinEventParams field to your MinEventParams,
			//it will be updated every frame and set to the holding ItemValue
	...
	private void Start()
    {
		...
		gameObject.AddComponent<Timer>().start(0.5f, 30, onTimerTick, null);
    }
