...
class MedicProc
{
    ...
    static MethodInfo mtdinfo_cde = AccessTools.Method(typeof(Entity), nameof(Entity.CanDamageEntity), new Type[] { typeof(Int32) });
    static bool need_postfix = true;

    public static void FakeFireEvent(Entity entity, EntityAlive attacker, ItemValue damageItemValue, WorldRayHitInfo hitInfo, bool useInventory)
    {
        if(attacker is EntityPlayer && entity is EntityAlive)
        {
            MinEventParams context = attacker.MinEventContext;
            context.Other = entity as EntityAlive;
    ...
    private static IEnumerable<CodeInstruction> Transpiler_Hit_ItemActionAttack(IEnumerable<CodeInstruction> instructions)
    {
        ...
        {
            if (codes[i].opcode == OpCodes.Callvirt && codes[i].Calls(mtdinfo_cde))
            {
                codes.InsertRange(i + 2, new CodeInstruction[]
                {
                    new CodeInstruction(OpCodes.Ldloc_S, 30),
                    new CodeInstruction(OpCodes.Ldloc_S, 6),
                    new CodeInstruction(OpCodes.Ldarg_S, 25),
                    new CodeInstruction(OpCodes.Ldarg_1),
                    new CodeInstruction(OpCodes.Ldloc_S, 7),
                    CodeInstruction.Call(typeof(MedicProc), nameof(MedicProc.FakeFireEvent), new Type[] { typeof(Entity), typeof(EntityAlive), typeof(ItemValue), typeof(WorldRayHitInfo), typeof(bool) })
                });
                break;
            }
        }
        ...
        codes.InsertRange(0, new CodeInstruction[]
        {
            ...
            CodeInstruction.StoreField(typeof(MedicProc), "need_postfix")
        });
    ...
    [HarmonyPatch(typeof(ItemActionAttack), nameof(ItemActionAttack.Hit))]
    [HarmonyPostfix]
    private static void Postfix_Hit_ItemActionAttack(World _world, WorldRayHitInfo hitInfo, int _attackerEntityId, ItemValue damagingItemValue)
    {
        if (!need_postfix)
        {
            need_postfix = true;
        ...
        if (hitInfo.tag.StartsWith("E_"))
        {
            ...
            if (entity != null && entity.entityId != _attackerEntityId && !entity.CanDamageEntity(_attackerEntityId))
            {
                ...
                FakeFireEvent(entity, attacker, damagingItemValue, hitInfo, useInventory);
            }
        }
    }
}
